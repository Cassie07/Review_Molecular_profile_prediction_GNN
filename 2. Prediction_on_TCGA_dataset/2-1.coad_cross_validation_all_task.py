...
task_list = ['Gene','Gene','Gene','CNA','CNA','CNA', 'MSI', 'Protein','Protein','Protein']
gene_list = ['lrp1b','fat4','ryr2','PLAGL2','DEFB118','REM1', 'MSI', 'EGFR_pY1173','ATM','P53']
...
print(adj_path5)
dic_name_data1, data_name, slide_name_order1, dic_slide_patch1 = generate_dataset(pca_feature_dimension, feature_path1, feature_name_path1, adj_path1, use_knn_graph, pca_right_now)
print('finish load dataset1')
dic_name_data2, data_name, slide_name_order2, dic_slide_patch2 = generate_dataset(pca_feature_dimension, feature_path2, feature_name_path2, adj_path2, use_knn_graph, pca_right_now)
print('finish load dataset2')
dic_name_data3, data_name, slide_name_order3, dic_slide_patch3 = generate_dataset(pca_feature_dimension, feature_path3, feature_name_path3, adj_path3, use_knn_graph, pca_right_now)
print('finish load dataset3')
dic_name_data4, data_name, slide_name_order4, dic_slide_patch4 = generate_dataset(pca_feature_dimension, feature_path4, feature_name_path4, adj_path4, use_knn_graph, pca_right_now)
print('finish load dataset4')
dic_name_data5, data_name, slide_name_order5, dic_slide_patch5 = generate_dataset(pca_feature_dimension, feature_path5, feature_name_path5, adj_path5, use_knn_graph, pca_right_now)
...
# task
for gene,task in zip(gene_list, task_list):
    if task == 'MSI':
...
#     dic_name_data_list = [dic_name_data1]
    ...
    # =====================================================================================================================
        ...
        if len(train_pos) > len(train_neg):
            train_pos, train_neg = oversampling(train_pos, train_neg)
        else:
            train_neg, train_pos = oversampling(train_neg, train_pos)
    # =====================================================================================================================


        for dic_name_data, slide_name_order, dic_slide_patch in zip(dic_name_data_list, slide_name_order_list, dic_slide_patch_list):
            print()
            print('【{} : Fold {} (model {}) 】'.format(gene, k, m))
            ...
            for i in train_pos:
                name = name_pos[i]
                train_dataset.append(dic_name_data[name])
            ...
            print('pos/neg in test : {}'.format(len(test_pos)/len(test_neg)))
            print()
            ...
            for epoch in range(num_epochs):
                train_loss, train_acc= train(train_loader, epoch, model, optimizer, device, init_lr, num_epochs)
                test_loss, test_acc, pred_label, pred_score, epoch_label, epoch_slide_select_patch = test(test_loader, slide_name_order, dic_slide_patch, dataset_name = 'testing')
...
#        flag = pred_list[0]
    ...
    with open('./journal/1000_{}_pca{}_no_drop_10fold_lr1e-3_fold_pred_label.json'.format(task, gene,pca_feature_dimension), 'w') as outfile:
        json.dump(dic_multi_model_pred, outfile)
    with open('./journal/1000_{}_pca{}_no_drop_10fold_lr1e-3_fold_pred_score.json'.format(task, gene,pca_feature_dimension), 'w') as outfile:
    ...
    gene_ensemble_result[gene] = [acc, auc, recall, precision, specificity, f1_score, tp, tn, fp, fn]
    log = {'name': name_list,'ground_truth': ground_truth, 'pred_label': y_pred, 'score': y_score}
    # attn[gene] = dic_multi_model_attn
    result_gene[gene] = result_k # {'gene' : {'1' : {'1':[acc,auc,....], '2':[...]}, '2':{...}}}

    with open('./journal/1000_{}_pca{}_no_drop_10fold_lr1e-3_log.json'.format(task, gene,pca_feature_dimension), 'w') as outfile:
        json.dump(log, outfile)

    with open('./journal/1000_{}_pca{}_no_drop_10fold_lr1e-3_top20_selected_patch.json'.format(task, gene,pca_feature_dimension), 'w') as outfile:
        json.dump(dic_multi_model_attn, outfile)


stop = time.time()

for gene, result_k in result_gene.items():
    print('【{}】'.format(gene))
    for fold, model_dic in result_k.items():
        sum_acc = 0
        sum_auc = 0
        for num_model, r in model_dic.items():
            print(' in fold {} (model {}) : acc is {}, auc is {}, tp : {}, tn: {}, fp: {}, fn:{}'.format(fold, num_model, r[0],r[1],r[2],r[3],r[4],r[5]))
            sum_acc += r[0]
            sum_auc += r[1]
...
for gene in gene_list:
    ...
    print('tp : {}, tn: {}, fp: {}, fn:{}'.format(r[6],r[7],r[8],r[9]))
    print()
